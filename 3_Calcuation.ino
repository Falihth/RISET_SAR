/*
  Kode file ini merupakan perhitungan inverse kinematics pada kaki robot.
  kode ini krusial untuk menentukan bagaimana robot bergerak.
  Jika ada waktu riset yang lebih lama, mungkin bisa digunakan untuk melakukan riset pada kode di file ini.
*/
...
void dinamicMovement(byte fn)
{
  ...
  if (fn == 1)
    pos_z[fn] = in_addLength + in_pz / perStep * sl[fn];
  if (fn == 2)
    pos_z[fn] = in_pz / perStep * sl[fn];
  if (fn == 3)
    pos_z[fn] = -in_addLength + in_pz / perStep * sl[fn];
...
void cal_IK(float px, float py, float pz, float Pitch, float Roll, float yaw, int fn)
{
  ...
  bodyIK_x[fn] = (total_X[fn] * cos_rot_Z[fn] * cos_rot_Y[fn]) - (total_Z[fn] * cos_rot_Z[fn] * sin_rot_Y[fn]) - (total_Y[fn] * sin_rot_Z[fn]) - total_X[fn];

  bodyIK_z[fn] = ((total_X[fn] * cos_rot_X[fn] * sin_rot_Y[fn]) + (total_X[fn] * cos_rot_Y[fn] * sin_rot_Z[fn] * sin_rot_X[fn]) + (total_Z[fn] * cos_rot_Y[fn] * cos_rot_X[fn]) - (total_Z[fn] * sin_rot_X[fn] * sin_rot_Z[fn] * sin_rot_X[fn]) - (total_Y[fn] * cos_rot_Z[fn] * sin_rot_X[fn])) - total_Z[fn];

  bodyIK_y[fn] = ((total_X[fn] * sin_rot_Y[fn] * sin_rot_X[fn]) - (total_X[fn] * cos_rot_Y[fn] * cos_rot_X[fn] * sin_rot_Z[fn]) + (total_Z[fn] * cos_rot_Y[fn] * sin_rot_X[fn]) + (total_Z[fn] * cos_rot_X[fn] * sin_rot_Y[fn] * sin_rot_Z[fn]) + (total_Y[fn] * cos_rot_Z[fn] * cos_rot_X[fn])) - total_Y[fn];
